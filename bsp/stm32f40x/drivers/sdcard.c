...
static rt_size_t rt_sdcard_read(rt_device_t dev, rt_off_t pos, void* buffer, rt_size_t size)
{
    ...
    while(retry)
    {
                ...
                ((rt_uint32_t)buffer > 0x20080000))
        {
            ...
            {
                status = SD_ReadBlock((u8 *)_sdcard_buffer,(long long)(part.offset + index + pos) * factor,
                                       SECTOR_SIZE);
        ...
        else
        {
            if (size == 1)
            {
                status = SD_ReadBlock((u8 *)buffer,(long long)(part.offset + pos) * factor,
                                       SECTOR_SIZE);
            }
            else
            {
                status = SD_ReadMultiBlocks((u8 *)buffer,(long long)(part.offset + pos) * factor,
                                             SECTOR_SIZE, size);
...
static rt_size_t rt_sdcard_write (rt_device_t dev, rt_off_t pos, const void* buffer, rt_size_t size)
{
            ...
            ((rt_uint32_t)buffer > 0x20080000))
    {
        ...
        for (index = 0; index < size; index ++)
        {
            ...
            status = SD_WriteBlock((u8 *)_sdcard_buffer,(long long)(part.offset + index + pos) * factor,
                                    SECTOR_SIZE);
    ...
    else
    {
        if (size == 1)
        {
            status = SD_WriteBlock((u8 *)buffer,(long long)(part.offset + pos) * factor,
                                    SECTOR_SIZE);
        }
        else
        {
            status = SD_WriteMultiBlocks((u8 *)buffer,(long long)(part.offset + pos) * factor,
                                          SECTOR_SIZE, size);
        }
    }
...
//    // delay same time for SD card power up
    ...
    if (SD_Init() == SD_OK)
    {
      ...
      if (status != SD_OK) goto __return;
        ...
        status = SD_ReadBlock((u8*)sector,0,  512);
        if (status == SD_OK)
        {

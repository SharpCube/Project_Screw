...
static void delay_ms(rt_uint32_t cnt)   
{   
...
static void delay_us(rt_uint32_t cnt)   
{   
    volatile rt_uint32_t dl;   
    while(cnt--)   
    {   
...
//写寄存器函数   
//regval:寄存器值   
lcd_inline  void LCD_WR_REG(vu16 regval)   
{   
...
//写LCD数据   
//data:要写入的值   
lcd_inline  void LCD_WR_DATA(vu16 data)   
{   
...
//读LCD数据   
//返回值:读到的值   
lcd_inline  u16 LCD_RD_DATA(void)   
{   
    //vu16 ram;         //防止被优化   
    //ram = ;   
    //return ram;   
...
/*  设置像素点 颜色,X,Y */   
void rt_hw_lcd_set_pixel(const char* pixels, int x, int y)   
{   
   
    LCD_SetCursor(x, y);   
    LCD_WriteRAM_Prepare();   
    LCD_WriteRAM(*(uint16_t *)pixels);     
}   
   
   
/* 获取像素点颜色 */   
void rt_hw_lcd_get_pixel(char* pixels, int x, int y)   
{   
   *(rt_uint16_t*)pixels= LCD_ReadPoint(x,y);   
}   
   
   
/* 画水平线 */   
void rt_hw_lcd_draw_hline(const char* pixels, int x1, int x2, int y)   
{   
...
/* 垂直线 */   
void rt_hw_lcd_draw_vline(const char* pixels, int x, int y1, int y2)   
{   
